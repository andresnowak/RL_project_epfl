...
if __name__ == "__main__":
    for size in [500, 1000, 2000, 3000]:
        ...
        for seed in [0, 1, 2]:
            ...
            preferences = load_trajectories(f"../rollouts/{ENV_NAME}", size, seed, ENV_NAME)
            ...
            agent = PPORLHFAgent(env, "cpu", f"../checkpoints/half_actor_model_{ENV_NAME}", n_epochs=10, entropy_coef=0.001, lr_reward=1e-3, reward_model_hidden_size=[64, 64])
            ...
            # ========== TRAINING ==========
            print("\n=== Training ===")
            reward_metrics = agent.train_reward_model(train_prefs, val_prefs, n_epochs=100, batch_size=32)

            save_agent_metrics(reward_metrics, size, seed, MODEL_SAVE_DIR, "reward", ENV_NAME)


            actor, critic, reward_net, metrics = agent.train(100)
            save_agent_metrics(metrics, size, seed, MODEL_SAVE_DIR, "agent", ENV_NAME)
            ...
            record_env = gym.wrappers.RecordVideo(
                record_env, f"videos_after_training_{ENV_NAME}/"
            )
            visualize_policy(agent.actor, record_env, 3)
            ...
            torch.save(
                {
                    ...
                    os.path.join(MODEL_SAVE_DIR, "reward_model"): reward_net.state_dict(),
                },
