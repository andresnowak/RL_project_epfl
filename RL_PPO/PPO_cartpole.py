...
class ActorNetwork(nn.Module):
    def __init__(self, n_actions, input_dims, alpha=1e-3, fc_dims=256, chkpt_dir="RL_PPO/model/"):
        super(ActorNetwork, self).__init__()

        self.checkpoint_file = os.path.join(chkpt_dir, "actor_torch_ppo")
        self.actor = nn.Sequential(nn.Linear(*input_dims, fc_dims), nn.ReLU(), nn.Linear(fc_dims, n_actions), nn.Softmax(dim=-1))
...
if __name__ == "__main__":
    ...
    plot_learning_curve(x, score_history, "RL_PPO/learning_curve_cartpole.png")
    ...
    torch.save(best_model, "RL_PPO/model/best_model_car_pole")
    print("save half model with score of", avg_score_history[half_idx])
    torch.save(half_model, "RL_PPO/model/half_model_car_pole")

    # visualize the best model

    env = gym.make("CartPole-v1", render_mode="human")
    agent = PPO(n_actions=env.action_space.n, input_dims=env.observation_space.shape)
    agent.load_models("RL_PPO/model/best_model_car_pole")
